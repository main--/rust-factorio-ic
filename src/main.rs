...
"#,
    ...
    for file in std::fs::read_dir(
        "/home/morpheus/.steam/steam/steamapps/common/Factorio/data/base/prototypes/recipe",
    )
    .unwrap()
    {
...
fn gridrender_subtree(
    subtree: &ProductionGraph, grid_i: &mut i32, pcb: &mut Vec<Entity>,
...
fn main() {
    ...
    let tree = kirkmcdonald(&recipes, "logistic-science-pack", 10.0);
    ...
    let (lins, lout) =
        gridrender_subtree(&tree, &mut grid_i, &mut pcb, &mut needed_wires, gridsize).unwrap();
